...
T3Pattern::T3Pattern(int p):Pattern(p){}

vector<Square*> T3Pattern::check(int r, int c, const Grid& g) {
  ...
  /*
    ...
    xxx
  */
  if(g.getSquare(r+1, c, color) && g.getSquare(r+2, c, color) && g.getSquare(r+2, c-1, color) && g.getSquare(r+2, c+1, color)){
  ...
  */
  ...
  /*
    ...
    xxo
  */
  else if(g.getSquare(r-1, c-1, color) && g.getSquare(r-2, c-1, color) && g.getSquare(r, c-1, color) && g.getSquare(r, c-2, color)){
    output.push_back(start);
    output.push_back(g.getSquare(r-2, c-1, color));
    output.push_back(g.getSquare(r-1, c-1, color));
...
pair<int, int> T3Pattern::newPos(int r, int c){
  return make_pair(r+1, c);
}

string T3Pattern::newType(){
  return "unstable";
}

Pattern* T3Pattern::copy(){
  T3Pattern* copy = new T3Pattern(priority);
  return copy;
}